...
impl Conversation {
    ...
    #[cfg(feature = "functions")]
    pub fn add_function<A: FunctionArgument + Send + Sync + 'static, C: CallableAsyncFunction<A> + Send + Sync + 'static>(&mut self, prebuilt: GptFunction<A, C>) -> crate::Result<()> {
        self.function_descriptors.push(serde_json::to_value(&prebuilt.descriptor).map_err(crate::err::Error::from)?);
        self.functions.insert(prebuilt.descriptor.name.to_owned(), Box::new(prebuilt));
        Ok(())
    }
    ...
    pub async fn send_role_message<S: Into<String> + Send + Sync>(
        ...
        #[cfg(feature = "functions")]
        let resp = if self.always_send_functions { self.client.send_history_functions(&self.history, &self.function_descriptors).await? } else { self.client.send_history(&self.history).await? };
        ...
        if let Some(function_response) = self.process_possible_function_response(msg).await {
            self.history.push(function_response.message_choices[0].message.clone());
    ...
    pub async fn send_message_functions<S: Into<String>>(
        ...
        let resp = self.client.send_history_functions(&self.history, &self.function_descriptors).await?;
        let msg = &resp.message_choices[0].message;
        self.history.push(msg.clone());
        if let Some(function_response) = self.process_possible_function_response(msg).await {
            self.history.push(function_response.message_choices[0].message.clone());
